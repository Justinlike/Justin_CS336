...
def main():
    ...
    parser.add_argument("--norm_mode", type=str, choices=["pre", "post"], default="pre")
    # Ablation 3: remove RoPE
    ...
    parser.add_argument("--ffn_type", type=str, default="swiglu", choices=["swiglu", "silu"],
                        help="Type of Feed-Forward Network")
    ...
    # np.memmap 延迟加载数据到内存，非常适合大数据集，并且将二进制文件转为 的type(uint16)数组
    ...
    print(f"验证集大小 {len(train_data)} tokens")
    ...
    print(f"Model Config: Norm={args.norm_mode}, RMSNorm={use_rms_norm}, FFN={args.ffn_type}")
    ...
    # 5. 检查点恢复逻辑
    start_iter = 0
    check_path = os.path.join(args.out_dir, "check.pt")
    if os.path.exists(check_path):
        start_iter = load_checkpoint(check_path, model, optimizer)
    ...
    for it in range(start_iter, args.max_iters):
        # A. update lr
        lr = get_lr_cosine_schedule(it, args.lr, args.min_lr, args.warmup_iters, args.warmup_iters)
        ...
        if it % 1000 == 0 and it > 0:
            save_checkpoint(model, optimizer, it, check_path)

        # 训练结束保存最终模型
        save_checkpoint(model, optimizer, args.max_iters, os.pat.join(args.out_dir, "final_check.pt"))
        wandb.finish()
