    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                # 这一步是为了消除初始化为 0 带来的偏移
                bias_corrention1 = 1 - beta1 ** t
                bias_corrention2 = 1 - beta2 ** t
                step_size = lr * (math.sqrt(bias_corrention2) / bias_corrention1)
                ...
                # p.add_(ather, alpha=1.0) p = p + (alpha * ather)
                if wd != 0:
...
def clip_gradient_norm(parameters: Iterable[torch.nn.Parameter], max_norm: float):
    ...
    :param parameters: 可迭代的参数列表 （通常是 model.parameters()）
    :param max_norm: 允许的最大梯度的 L2 范数 （M）
    想象你在下山（优化模型），正常的步子很稳。但突然遇到一个极陡的悬崖（梯度爆炸），如果不加控制，你这一步跨出去可能会直接飞出景区（权重更新过大，Loss 变成 NaN）。
    梯度裁剪就是一个“自动刹车系统”，会检测所有步子的总长度，如果超过了安全阈值 M，就把步子按比例缩小
